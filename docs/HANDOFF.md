...
- Fresh validation: `pnpm test` 101/101 PASS; `pnpm exec tsc --noEmit`, `pnpm lint`, `pnpm audit:seo`, `pnpm audit:ads`, `pnpm audit --prod`, and `git diff --check` PASS. The regenerated static sitemap contains no rescue, OpenGraph image, icon, apple-icon, or manifest URLs. A fresh schema-complete build generated 72 pages without the former Next.js warnings. Local production HTTP returned 200 for `/feed.xml` and `/blog`, with no unsupported expert-review phrase in either response; the test server was stopped.
