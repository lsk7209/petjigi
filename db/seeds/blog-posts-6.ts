...
const BLOG_POSTS_6: NewContent[] = [
  // ── 51. 고양이 물 섭취 (cat2, macro:B, lens:L4, hook:H2, outro:O3) ──
  {
    ...
    body: `<p>고양이는 진화적으로 사막 출신이라 갈증을 잘 느끼지 못하도록 설계된 동물이다. 야생에서는 먹잇감의 수분(약 70%)으로 대부분의 수분을 충당했다. 문제는 건식 사료 위주로 사는 실내 고양이다. 건사료의 수분 함량은 10% 미만, 고양이는 음수로 부족분을 채워야 하는데 좀처럼 물을 마시지 않는다.</p>

<h2>하루 적정 물 섭취량</h2>
<p>Cornell Feline Health Center에 따르면 고양이의 하루 적정 수분 섭취량은 체중 1kg당 약 60ml다. 4kg 고양이라면 하루 240ml가 기준이다. 단 이는 총 수분량이므로 습식 사료를 먹으면 그만큼 음수 필요량은 줄어든다. 습식 사료의 수분 함량은 75~80%이므로 습식을 급여하면 자연스럽게 수분 보충이 이루어진다.</p>
...
<p>갑자기 물을 과도하게 많이 마시거나(다음증), 반대로 완전히 마시지 않는 증상이 지속되면 내분비 질환이나 신장 문제의 신호일 수 있어 동물병원 진찰이 필요하다. 고양이 건강에 대한 더 많은 정보는 <a href="/condition">질환 정보 페이지</a>에서 확인할 수 있다.</p>`,
  ...
  // ── 52. 강아지 영양제 가이드 (cat2, macro:C, lens:L2, hook:H1, outro:O1) ──
  {
...
<h3>오메가-3 (EPA/DHA)</h3>
<p>생선 기반 오메가-3는 피부 건강, 코트 윤기, 관절 염증 완화에 근거가 있는 보조제다. 상업 사료에도 오메가-3가 포함돼 있지만 산화가 빨라 사료 내 함량이 보장치에 못 미치는 경우가 많다. 피부 문제나 관절 불편이 있는 강아지에게 수의사 지도하에 보충을 고려할 수 있다. 단, 생선 알러지 강아지는 주의한다.</p>
...
<h3>종합 비타민</h3>
<p>완전 영양 사료를 먹는 강아지에게 종합 비타민은 대부분 불필요하며, 지용성 비타민(A, D, E, K) 과다 보충은 독성을 유발할 수 있다. 특히 비타민 D 과다는 고칼슘혈증으로 이어질 수 있어 주의가 필요하다.</p>
...
<p>영양제에 돈을 쓰기 전에 현재 급여하는 사료가 영양학적으로 완전한지 먼저 점검하는 것이 순서다. <a href="/blog/dog-food-by-age">나이별 강아지 사료 선택 가이드</a>와 <a href="/blog/cat-food-label-guide">원료 표시 읽는 법</a>을 참고해 사료 기반을 탄탄히 다지자.</p>`,
  ...
  // ── 53. 노령묘 사료 가이드 (cat2, macro:B, lens:L5, hook:H4, outro:O2) ──
  {
...
<ul>
  <li><strong>단백질 소화·흡수 효율 저하</strong>: 노령묘는 같은 양의 사료에서 흡수하는 단백질이 성묘보다 적다. 오히려 단백질 함량이 높은 사료가 필요한 이유다.</li>
  <li><strong>칼로리 요구량 변화</strong>: 11세 이상 초고령묘는 칼로리 흡수 능력이 더 떨어져 오히려 칼로리를 높여야 한다는 연구가 있다. 반면 7~11세 구간은 비만 위험도 있으므로 체중 모니터링이 중요하다.</li>
  <li><strong>신장 기능 저하</strong>: 노령묘의 가장 흔한 질환은 만성 신부전(CKD)이다. 인(phosphorus) 섭취를 관리하는 것이 신장 건강 보호에 중요하다.</li>
  <li><strong>치아·구강 변화</strong>: 치주 질환이 있는 노령묘는 건사료를 씹기 힘들어 습식이나 반습식으로 전환이 필요할 수 있다.</li>
</ul>

<h2>노령묘 사료 선택 기준</h2>

<h3>단백질</h3>
<p>WSAVA는 노령묘에게 건물기준(DM) 단백질 함량 36% 이상을 권고한다. 신장 질환이 없는 한 단백질 제한이 필요하지 않다. 오히려 단백질 부족이 근감소증(사코페니아)을 촉진한다. 동물성 단백질(닭, 칠면조, 생선)이 첫 번째 원료로 표기된 제품을 선택한다.</p>
...
<p>시판 시니어 사료의 영양 성분이 실제로 노령묘에게 더 적합한지는 제품마다 다르다. "Senior" 표기는 규제 기준이 없으므로 라벨을 직접 비교하는 것이 중요하다. <a href="/blog/cat-food-label-guide">사료 원료 표시 읽는 법</a>을 알면 비교가 쉬워진다.</p>
...
<p>만성 신부전, 갑상선 기능항진증, 당뇨, 심장 질환이 진단된 경우 수의사의 처방식 지도가 필요하다. 임의로 처방식을 구매해 급여하는 것은 오히려 독이 될 수 있다.</p>`,
  ...
  // ── 54. 강아지 칼로리 계산 (cat2, macro:F, lens:L3, hook:H5, outro:O4) ──
  {
    ...
    body: `<p>강아지 10마리 중 5~6마리가 과체중 또는 비만이라는 미국수의사협회(AVMA) 통계가 있다. 과체중은 관절 질환, 당뇨, 심장 질환, 수명 단축으로 이어진다. 비만 예방의 시작은 하루 필요 칼로리를 정확히 파악하는 것이다.</p>

<h2>칼로리 계산 기본 공식</h2>

<h3>RER (안정 에너지 요구량)</h3>
<p>RER = 70 × (체중kg)<sup>0.75</sup></p>
<p>이는 강아지가 완전히 쉴 때 생명 유지에 필요한 최소 칼로리다.</p>
...
<p>단, 간식도 칼로리에 포함해야 한다. 하루 총 칼로리의 10% 이상을 간식이 차지하면 사료 급여량을 줄여야 한다.</p>

<h2>체중 모니터링</h2>
<p>월 1회 체중을 측정해 기록한다. 갑자기 체중이 늘거나 줄면 급여량 조절 또는 동물병원 상담이 필요하다. <a href="/blog/dog-weight-management">강아지 체중 관리 가이드</a>에서 더 자세한 방법을 확인할 수 있다.</p>

<h2>사료 포장지의 급여 지침만 믿으면 안 되는 이유</h2>
<p>사료 제조사의 권장 급여량은 중성화하지 않은 성견 기준으로 설정된 경우가 많아 중성화 강아지에게는 과할 수 있다. 강아지의 BCS(체형 점수)를 기준으로 실제 급여량을 조절하는 것이 더 정확하다. 수의사에게 BCS 체크 방법을 물어보자.</p>`,
    ymyl: false,
    status: "published",
  ...
  // ── 55. 고양이 먹으면 안 되는 음식 (cat2, macro:B, lens:L4, hook:H2, outro:O3) ──
  {
    ...
    body: `<p>고양이는 독립적이고 까다로운 미식가처럼 보이지만, 실제로는 인간의 음식 중 심각한 독성을 가진 것들이 생각보다 많다. "조금은 괜찮겠지"라는 생각이 응급실행으로 이어지는 경우가 있다.</p>

<h2>절대 주면 안 되는 음식</h2>

<h3>파류 (양파·마늘·대파·부추)</h3>
<p>파과 식물에 포함된 N-프로필 디술피드 화합물이 고양이의 적혈구를 파괴해 용혈성 빈혈을 유발한다. 익혀도 독성이 사라지지 않는다. 고양이는 개보다 훨씬 민감하며 소량도 위험하다. 증상: 무기력, 식욕 부진, 잇몸 창백, 호흡 곤란.</p>

<h3>포도와 건포도</h3>
<p>정확한 독성 메커니즘은 아직 연구 중이나, 포도와 건포도는 고양이·개에게 급성 신부전을 유발할 수 있다. 미국동물독성관리센터(ASPCA Animal Poison Control)는 포도류를 고위험 식품으로 분류한다.</p>

<h3>초콜릿·카카오</h3>
<p>테오브로민과 카페인이 신경계·심장에 독성을 나타낸다. 다크 초콜릿과 무가당 코코아파우더가 가장 위험하다. 증상: 구토, 설사, 빠른 심박, 경련.</p>
...
<h3>알코올</h3>
<p>아주 소량도 간·신장 손상, 신경계 억제를 유발한다. 취하게 만드는 것은 절대 해서는 안 되는 행동이다.</p>

<h3>날생선·날고기 (일부)</h3>
<p>날생선에 포함된 티아미나아제 효소는 비타민 B1(티아민)을 파괴해 신경 문제를 유발할 수 있다. 정기적으로 급여하면 위험하다. 단, 위생적으로 처리된 생식(BARF) 식단은 수의사 지도하에 진행하는 것이 원칙이다.</p>
...
<p>고양이 건강 정보는 <a href="/condition">질환 정보</a> 페이지에서 더 자세히 확인할 수 있으며, <a href="/blog/dog-toxic-foods">강아지 독성 음식 목록</a>도 함께 참고하자.</p>`,
  ...
  // ── 56. 강아지 채소 과일 가이드 (cat2, macro:B, lens:L1, hook:H3, outro:O2) ──
  {
...
<ul>
  <li><strong>당근</strong>: 칼로리가 낮고 섬유질이 풍부. 생으로 씹으면 치아 관리에도 도움. 단, 크게 자른 것은 질식 위험이 있으므로 작게 잘라 준다.</li>
...
<ul>
  <li><strong>사과</strong>: 씨앗을 반드시 제거한 후 준다. 씨앗에 시안화물(청산가리) 화합물이 있다. 껍질 포함 과육은 안전하다.</li>
...
<h2>절대 주면 안 되는 것</h2>
<ul>
  <li><strong>포도·건포도</strong>: 급성 신부전 유발 가능. 단 1알도 위험하다.</li>
  <li><strong>양파·마늘·대파</strong>: 용혈성 빈혈. 생것·익힌 것·분말 모두 위험.</li>
  <li><strong>아보카도</strong>: 퍼신(persin) 성분이 심근·호흡 문제를 유발할 수 있다.</li>
  <li><strong>체리</strong>: 씨·잎·줄기에 시안화물 화합물 포함.</li>
  <li><strong>무화과</strong>: 피부·구강 자극 및 위장 장애 유발.</li>
</ul>

<h2>채소·과일 급여 원칙</h2>
<p>어떤 채소·과일이든 하루 총 칼로리의 10%를 넘지 않아야 한다. 처음 주는 음식은 소량부터 시작해 반응을 관찰한다. 구토, 설사, 복부 팽만이 나타나면 해당 식품을 중단하고 지속되면 동물병원을 방문한다. <a href="/blog/dog-toxic-foods">강아지 먹으면 안 되는 음식 전체 목록</a>도 함께 확인하자.</p>`,
    ymyl: false,
    status: "published",
  ...
  // ── 57. 고양이 간식 가이드 (cat2, macro:C, lens:L2, hook:H1, outro:O1) ──
  {
    ...
    body: `<p>고양이 간식 시장은 해마다 빠르게 성장하고 있다. 반려묘와의 유대를 위해, 또는 약 먹이기 위해, 훈련 보상으로 간식을 주는 보호자가 많다. 하지만 간식이 주식 영양을 방해하거나 과도한 칼로리를 추가하면 오히려 건강을 해칠 수 있다.</p>

<h2>10% 규칙 — 간식의 황금률</h2>
<p>WSAVA는 하루 총 칼로리의 10% 이상을 간식이 차지하면 주식의 영양 균형이 깨질 수 있다고 경고한다. 4kg 고양이의 하루 칼로리가 약 240kcal라면 간식은 24kcal 이내가 기준이다. 작은 고양이 트릿 하나가 2~5kcal임을 감안하면, 하루 5~10개 정도가 한계인 경우가 많다.</p>
...
<p>원료의 영양소를 가장 잘 보존하는 방식이다. 단백질 함량이 높고 첨가물이 적은 경우가 많다. 가격이 다소 높지만 성분이 단순하다.</p>

<h3>액상 간식(츄르 등)</h3>
<p>국내에서 매우 인기 있는 형태. 수분 보충에도 도움이 된다. 단, 성분표에서 소금(나트륨) 함량을 확인한다. 일부 제품은 나트륨이 높아 신장에 부담을 줄 수 있다. 하루 1~2개 이내로 제한하는 것이 좋다.</p>

<h3>덴탈 간식</h3>
<p>치아 관리 효과를 내세우는 제품들이다. VOHC(수의구강건강위원회) 인증 마크가 있으면 임상적으로 효과가 검증된 것이다.</p>

<h2>피해야 할 성분</h2>
<ul>
  <li>프로필렌 글리콜 (고양이에게 독성)</li>
...
<p>고양이 클리커 훈련이나 약 먹이기에 간식을 활용할 때는 작은 크기로 쪼개 보상 횟수를 늘리면 총 칼로리를 줄이면서 효과는 극대화할 수 있다. <a href="/blog/cat-clicker-training-guide">고양이 클리커 훈련 가이드</a>를 참고하자.</p>`,
  ...
  // ── 58. 강아지 수분 섭취 가이드 (cat2, macro:B, lens:L5, hook:H2, outro:O3) ──
  {
...
<h2>하루 적정 물 섭취량</h2>
<p>미국수의사협회(AVMA) 기준, 강아지의 하루 물 섭취량은 체중 1kg당 약 50~60ml가 기준이다. 5kg 강아지는 하루 250~300ml, 10kg 강아지는 500~600ml가 목표다. 단, 습식 사료를 먹거나 날씨가 더울 때, 운동량이 많을 때는 필요량이 늘어난다.</p>
...
<h3>잇몸 확인</h3>
<p>잇몸이 촉촉하고 분홍색이어야 정상이다. 끈적하거나 창백하다면 탈수 가능성이 있다. 손가락으로 잇몸을 눌렀다가 떼면 하얘졌다가 2초 이내에 분홍색으로 돌아오는 것이 정상(모세혈관 충전 시간)이다.</p>
...
<h2>언제 동물병원에 가야 하나</h2>
<p>갑자기 물을 엄청나게 많이 마시는 다음증(polydipsia)은 당뇨병, 신부전, 쿠싱 증후군 등 내분비 질환의 신호일 수 있다. 반대로 물을 전혀 마시지 않거나 구토·설사를 동반한 무기력증이 있다면 지체 없이 동물병원을 찾아야 한다. 더 넓은 영양·건강 정보는 <a href="/guide">펫지기 가이드</a>에서 확인하자.</p>`,
    ymyl: false,
    status: "published",
  ...
  // ── 59. 강아지 사료 보관법 (cat2, macro:C, lens:L3, hook:H5, outro:O1) ──
  {
    ...
    body: `<p>좋은 사료를 샀어도 보관을 잘못하면 영양소가 파괴되고 산화·오염이 일어난다. 산화된 지방은 소화기 장애를 유발하고, 곰팡이가 피면 독소(아플라톡신)가 생성돼 간 손상을 일으킬 수 있다. 사료 보관은 단순한 위생 문제가 아니라 건강 문제다.</p>

<h2>건식 사료 보관 원칙</h2>

<h3>원래 봉투에 담아 밀봉</h3>
<p>많은 보호자가 사료 전용 용기에 사료를 쏟아 담는다. 하지만 사료 봉투 안쪽에는 산소·빛 차단 코팅이 되어 있어, 원래 봉투 그대로 밀봉 클립으로 막아 용기에 넣는 것이 가장 좋다. 용기에 그냥 쏟으면 잔여 유지가 용기에 남아 새 사료에 산화균이 옮겨붙을 수 있다.</p>

<h3>서늘하고 건조한 곳에 보관</h3>
<p>온도 21°C 이하, 습도 50% 이하가 이상적이다. 냉장 보관은 결로 때문에 습기가 생겨 오히려 곰팡이 위험이 높아진다. 햇빛이 직접 닿는 곳, 오븐 옆, 냉장고 위처럼 온도 변화가 심한 곳도 피한다.</p>

<h3>개봉 후 6주 이내 소진</h3>
<p>아무리 잘 밀봉해도 개봉 후 시간이 지나면 산화가 진행된다. 대형견이 아니라면 큰 포대보다 중간 크기 포대 여러 개를 사는 것이 신선도 면에서 유리하다.</p>
...
<p>보관을 잘해도 원재료 품질이 낮으면 의미가 없다. <a href="/blog/dog-food-by-age">나이별 강아지 사료 선택 가이드</a>와 <a href="/blog/dog-food-allergy-diet">강아지 식이 알러지 관리</a>를 함께 읽어보자.</p>`,
  ...
  // ── 60. 고양이 처방식 가이드 (cat2, macro:B, lens:L4, hook:H4, outro:O2) ──
  {
    ...
    body: `<p>처방식(Prescription Diet 또는 Therapeutic Diet)은 특정 질환을 관리하기 위해 영양소 조성을 조절한 사료다. 수의사의 진단과 처방 없이 임의로 구매해 먹이는 것은 오히려 건강을 해칠 수 있다.</p>

<h2>처방식이 필요한 주요 질환</h2>

<h3>만성 신부전 (CKD)</h3>
<p>신장 기능이 저하된 고양이에게는 인(phosphorus)과 단백질 함량을 낮춘 처방식이 필요한 경우가 있다. 단, IRIS(국제신장학회)의 병기 분류에 따라 필요한 영양 조절이 다르기 때문에 수의사 지도가 필수다. 초기(IRIS 1~2기)에는 단백질 제한보다 수분 보충과 인 관리가 우선이다.</p>

<h3>하부요로계 질환 (FLUTD)</h3>
<p>결석 유형(스트루바이트 vs 옥살산칼슘)에 따라 처방식의 방향이 완전히 달라진다. 스트루바이트 결석은 소변을 산성화하는 처방식이 필요하고, 옥살산칼슘 결석은 반대 방향의 조절이 필요하다. 결석 유형 분석 없이 임의로 처방식을 고르면 역효과가 난다. 관련 질환 정보는 <a href="/condition">질환 정보 페이지</a>에서 확인하자.</p>

<h3>당뇨</h3>
<p>고단백·저탄수화물 식단이 혈당 관리에 도움이 된다. 시판 처방 당뇨식 외에도 고단백 습식 사료가 대안이 될 수 있다. 인슐린 치료 중인 고양이는 식사 변화가 혈당에 직접 영향을 주므로 사료 전환 시 반드시 수의사와 상의한다.</p>

<h3>갑상선 기능항진증</h3>
<p>요오드를 극도로 제한한 처방식이 갑상선 호르몬 생성을 억제하는 데 쓰인다. 단, 이 처방식은 다른 음식이나 간식을 전혀 주지 않아야 효과가 있다. 처방식 이외의 모든 음식 차단이 필수다.</p>

<h2>처방식 전환 방법</h2>
<p>갑자기 사료를 바꾸면 고양이가 거부하거나 소화기 장애가 생길 수 있다. 7~14일에 걸쳐 서서히 비율을 바꾼다:</p>
<ul>
