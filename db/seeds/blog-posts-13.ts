...
const BLOG_POSTS_13: NewContent[] = [
  // ── 121. 아이에게 반려동물 죽음 설명하기 (cat6) ──
  {
    ...
    metaDescription: "3세부터 10세 이상까지, 아이의 발달 단계별 반려동물 죽음 설명법. '잠들었다' 완곡어법의 문제, 솔직한 설명의 중요성, 아이와 함께하는 추모 방법.",
...
<p>→ 가족 모두가 함께 슬퍼하는 모습을 보여주는 것이 아이에게 "슬픔은 부끄러운 것이 아니다"라는 메시지를 전달하는 가장 좋은 방법이다. 추모 과정에 대해서는 <a href="/blog/pet-memorial-ideas">반려동물 추모 아이디어</a>를 참고하면 함께할 수 있는 방법을 더 찾을 수 있다.</p>`,
    status: "published",
    ymyl: true,
  ...
  // ── 122. 반려동물 호스피스 케어 가이드 (cat6) ──
  {
    ...
    metaDescription: "반려동물 호스피스 케어의 의미와 시작 시점, 재가 호스피스 환경 구성법, 통증 신호 파악, 수의사와 협력하는 방법. 마지막까지 편안하게 함께하는 법.",
...
<p>마지막 징후에 대해 더 알고 싶다면 <a href="/blog/pet-end-of-life-signs">반려동물 마지막 징후 알아보기</a>를 참고하고, 안락사 결정에 대해서는 <a href="/blog/pet-euthanasia-guide">반려동물 안락사 가이드</a>를 읽어보시기 바란다.</p>`,
  ...
  // ── 123. 반려동물 화장 vs 장묘 비교 (cat6) ──
  {
    ...
    metaDescription: "반려동물 화장(개별/합동)과 장묘(사설공원·자가매장) 비용과 절차 비교. 한국 법적 기준, 선택 시 고려 사항, 추모공원 이용 방법까지.",
...
<p>화장 이후 추모공원 안치를 고려 중이라면 <a href="/blog/pet-memorial-park-guide">반려동물 추모공원 이용 가이드</a>를 참고하고, 유골을 활용한 다양한 추모 방식은 <a href="/blog/pet-memorial-ideas">반려동물 추모 아이디어</a>에서 찾아볼 수 있다.</p>`,
  ...
  // ── 124. 펫로스 증후군 극복 가이드 (cat6) ──
  {
    ...
    metaDescription: "반려동물을 잃은 후 찾아오는 펫로스 증후군의 증상·원인·회복 과정. 슬픔의 5단계, 일상 회복 방법, 전문 상담이 필요한 시점.",
...
<p>새 반려동물을 맞이하는 시기에 대한 고민이 있다면 <a href="/blog/when-to-get-new-pet-after-loss">새 반려동물을 들이는 적절한 시기</a>를 참고하고, 아이와 함께 슬픔을 나누는 방법은 <a href="/blog/explaining-pet-death-to-children">아이에게 반려동물 죽음 설명하기</a>에서 볼 수 있다.</p>`,
  ...
  // ── 125. 반려동물 추모 아이디어 (cat6) ──
  {
    ...
    metaDescription: "반려동물을 기억하는 15가지 추모 방법. 수제 추모 물품, 유골 활용 주얼리, 추모 식물·나무, 온라인 추모관, 봉사·기부까지 다양한 방식 소개.",
...
<p>슬픔을 통과하는 과정에 대해서는 <a href="/blog/pet-grief-recovery-guide">펫로스 증후군 극복 가이드</a>를 참고하시기 바란다.</p>`,
  ...
  // ── 126. 새 반려동물을 들이는 시기 (cat6) ──
  {
    ...
    metaDescription: "반려동물을 잃고 나서 새 가족을 맞이하는 시기에 대한 고민. 정해진 답은 없지만, 자신이 준비됐는지 확인하는 5가지 신호와 주의할 점.",
...
<p>슬픔을 통과하는 과정에 대한 더 많은 정보는 <a href="/blog/pet-grief-recovery-guide">펫로스 증후군 극복 가이드</a>를 참고하고, 새 반려동물 입양 준비물은 <a href="/blog/dog-adoption-checklist">강아지 처음 입양 준비물 체크리스트</a>에서 확인할 수 있다.</p>`,
  ...
  // ── 127. 반려동물 유언장·유서 가이드 (cat6) ──
  {
    ...
    metaDescription: "보호자 사망·사고 시 반려동물을 보호하는 법. 한국에서의 반려동물 법적 지위, 유언장에 반려동물 포함하는 방법, 임시 보호 계획 세우기.",
...
<p>반려동물 관련 법률 전반에 대해서는 <a href="/blog/animal-protection-law-guide">동물보호법 완전 가이드</a>를 참고하시기 바란다.</p>`,
    status: "published",
    ymyl: true,
    disclaimer: CAT6_DISCLAIMER,
    sources: JSON.stringify([
  ...
  // ── 128. 반려동물 마지막 징후 (cat6) ──
  {
    ...
    metaDescription: "반려동물이 마지막을 앞두고 보내는 신체적·행동적 신호. 식욕 감소, 호흡 변화, 고립 행동 등 징후별 의미와 곁에서 해줄 수 있는 것.",
...
<p>반려동물이 숨을 거두면 수의사에게 연락해 사망 확인을 받는다. 이후 화장·장묘 등 배웅 방식에 대해서는 <a href="/blog/pet-cremation-vs-burial">반려동물 화장 vs 장묘 비교</a>를 참고하고, 호스피스 케어에 대해서는 <a href="/blog/pet-hospice-care-guide">반려동물 호스피스 케어 가이드</a>를 읽어보시기 바란다.</p>`,
  ...
  // ── 129. 반려동물 안락사 가이드 (cat6) ──
  {
    ...
    metaDescription: "반려동물 안락사 결정 기준, 절차, 비용, 함께할 수 있는지 여부. 결정 후 죄책감 다루는 법과 이후 슬픔 관리까지 솔직하게 안내합니다.",
...
<p>이후 슬픔에 대처하는 방법은 <a href="/blog/pet-grief-recovery-guide">펫로스 증후군 극복 가이드</a>를 참고하고, 마지막 배웅 방식은 <a href="/blog/pet-cremation-vs-burial">반려동물 화장 vs 장묘 비교</a>에서 확인할 수 있다.</p>`,
  ...
  // ── 130. 반려동물 추모공원 이용 가이드 (cat6) ──
  {
    ...
    metaDescription: "반려동물 추모공원의 종류(납골당·자연장·봉안묘), 비용, 허가 업체 확인 방법, 방문 추모까지. 올바른 추모공원 선택을 위한 체크리스트.",
