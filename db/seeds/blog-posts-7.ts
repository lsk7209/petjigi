...
const BLOG_POSTS_7: NewContent[] = [
  // ── 61. 강아지 설사 가이드 (cat3, YMYL, macro:A, lens:L4, hook:H2, outro:O3) ──
  {
...
<p>수의학적으로 설사는 소장성(small bowel)과 대장성(large bowel)으로 나뉘며, 특성이 다르다.</p>
<ul>
  <li><strong>소장성 설사</strong>: 양이 많고, 하루 1~3회, 색이 정상~노란색, 혈변 시 암적색(소화된 혈액). 탈수 위험이 높다.</li>
  <li><strong>대장성 설사</strong>: 양은 적지만 빈도가 잦음(하루 5회+), 점액이 섞이거나 선홍색 혈변, 이끼는 행동(테네스무스). 탈수 위험은 상대적으로 낮다.</li>
</ul>
...
<p>성견이고, 혈변·구토·무기력 없이 1~2회의 연변이 있는 경우는 일시적 소화 불량일 가능성이 높다. 이때 취할 수 있는 조치:</p>
<ul>
...
<h2>흔한 원인들</h2>
<p>미국수의내과학회(ACVIM) 자료에 따르면 급성 설사의 가장 흔한 원인은 식이 변화(갑자기 사료를 바꾸거나 낯선 음식 섭취), 스트레스, 장내 기생충, 바이러스 감염이다. 강아지 파보바이러스는 구토+혈성 설사+극심한 무기력이 특징이며 미접종 강아지에서 생명을 위협하는 응급 상황이다.</p>
...
<p>예방접종과 정기 건강검진은 바이러스성·기생충성 설사를 예방하는 가장 기본이다. <a href="/blog/dog-vaccination-schedule">강아지 예방접종 일정표</a>를 참고하자.</p>`,
    ymyl: true,
  ...
  // ── 62. 고양이 FLUTD 가이드 (cat3, YMYL, macro:B, lens:L4, hook:H4, outro:O2) ──
  {
...
<h2>FLUTD 주요 원인</h2>
<ul>
  <li><strong>특발성 방광염(FIC)</strong>: 원인 불명의 방광 염증으로 FLUTD의 60~70%를 차지한다. 스트레스가 주요 유발 인자다.</li>
  <li><strong>요로 결석(urolithiasis)</strong>: 스트루바이트 결석과 옥살산칼슘 결석이 가장 흔하다. 결석 유형에 따라 관리 방법이 다르다.</li>
...
<p>WSAVA와 유럽수의영양학저널(ESVCN) 연구에 따르면 FLUTD 예방에서 수분 섭취량 증가가 가장 근거 있는 방법이다. 소변 희석으로 결정 형성과 방광 자극을 줄인다.</p>
<ul>
...
<p>FLUTD 진단과 치료는 반드시 수의사와 함께 진행해야 한다. 결석 유형 분석 없이 처방식을 임의로 선택하면 역효과가 날 수 있다. 관련 처방식 정보는 <a href="/blog/cat-prescription-diet-guide">고양이 처방식 가이드</a>에서 확인하자.</p>`,
  ...
  // ── 63. 강아지 관절염 관리 (cat3, YMYL, macro:B, lens:L5, hook:H3, outro:O2) ──
  {
    ...
    body: `<p>미국수의내과학회(ACVIM) 연구에 따르면 1세 이상 개의 약 20%, 8세 이상 노령견의 약 80%에서 골관절염(OA, osteoarthritis) 징후가 관찰된다. 그러나 강아지는 통증을 잘 드러내지 않는다. 보호자가 이상을 감지했을 때 이미 어느 정도 진행된 경우가 많다.</p>
...
<p>ACVIM은 체중 감량이 관절염 관리에서 가장 효과적인 개입 중 하나라고 명시한다. 체중이 1kg 줄면 관절에 가해지는 힘이 약 4kg 감소한다. <a href="/blog/dog-weight-management">강아지 체중 관리 가이드</a>를 함께 참고하자.</p>
...
<p>관절염은 수의사 진단과 치료 계획이 필요한 질환이다. 일반적인 치료 방향은 통증 관리, 항염증 치료, 재활, 보조제 등을 종합적으로 활용하며 강아지의 상태에 따라 수의사가 결정한다. 관절염이 의심된다면 X-ray 검사를 포함한 진단이 필요하다. <a href="/insurance/compare">펫보험</a>을 통해 의료비 부담을 줄이는 방법도 확인해 보자.</p>`,
  ...
  // ── 64. 고양이 신부전 초기 신호 (cat3, YMYL, macro:A, lens:L4, hook:H5, outro:O3) ──
  {
    ...
    body: `<p>Cornell Feline Health Center에 따르면 7세 이상 고양이의 30~40%가 만성 신부전(CKD, Chronic Kidney Disease)을 경험한다. 가장 안타까운 것은 신장 기능의 75% 이상이 손실돼야 임상 증상이 나타난다는 점이다. 증상이 보일 때는 이미 상당히 진행된 상태다.</p>
...
<p>IRIS(국제신장학회)는 고양이 CKD를 BUN, Creatinine, SDMA 수치와 소변 비중을 기반으로 1~4기로 분류한다. SDMA는 신장 기능의 25% 손실 시점에서 이미 상승해 Creatinine보다 훨씬 일찍 이상을 감지한다. 7세 이상 고양이는 연 2회 혈액·소변 검사를 통해 신장 수치를 모니터링하는 것이 WSAVA 권고 사항이다.</p>
...
<p>CKD는 현재 기술로 완치가 어렵지만 적절한 관리로 삶의 질을 유지하며 오랜 시간을 함께할 수 있다. 수의사와 정기적인 추적 검사 일정을 세우고, 집에서 할 수 있는 모니터링(체중 측정, 음수량 기록, 식욕 관찰)을 습관화하자. 노령묘 사료 선택은 <a href="/blog/senior-cat-food-guide">노령묘 사료 가이드</a>도 함께 참고한다.</p>`,
  ...
  // ── 65. 강아지 유치 가이드 (cat3, YMYL, macro:C, lens:L1, hook:H3, outro:O1) ──
  {
...
<ul>
  ...
  <li><strong>생후 7개월</strong>: 영구치(42개) 교환 완료가 목표</li>
</ul>
...
<p>영구치가 올라왔는데 유치가 빠지지 않아 두 개의 치아가 겹쳐 있는 상태를 이중치 또는 지속 유치(retained deciduous teeth)라고 한다. 소형견·장두종에서 더 흔하다. AVDC(미국수의치과학회)에 따르면 이중치를 방치하면 치아 사이에 음식물이 끼고, 치석·치주 질환·영구치 위치 이상으로 이어질 수 있다. 영구치가 나온 후 2주 이상 유치가 남아 있다면 수의사 확인이 필요하다.</p>
...
<p>칫솔질 습관을 들이는 방법은 <a href="/blog/dog-dental-care">강아지 치아 관리 가이드</a>에서 자세히 확인할 수 있다.</p>`,
    ymyl: true,
    disclaimer: YMYL_DISCLAIMER,
    status: "published",
  ...
  // ── 66. 고양이 갑상선 기능항진증 (cat3, YMYL, macro:B, lens:L4, hook:H2, outro:O3) ──
  {
    ...
    body: `<p>10세 이상 고양이를 키우는 보호자라면 한 번쯤 들어봤을 질환이다. 밥도 잘 먹고 활발한 것 같은데 몸무게는 계속 줄어든다. 갑상선 기능항진증(Hyperthyroidism)은 10세 이상 고양이에서 가장 흔한 호르몬 질환으로, Cornell Feline Health Center 자료에 따르면 10세 이상 고양이의 약 10%에서 발생한다.</p>
...
<h2>진단</h2>
<p>혈액검사에서 T4 수치가 높게 나오면 진단할 수 있다. T4가 경계 수치라면 더 민감한 fT4 검사를 추가하기도 한다. 갑상선 기능항진증은 만성 신부전을 가리는 효과가 있어, 치료 후 갑자기 신장 수치가 나빠지는 경우도 있으므로 신장 기능도 함께 모니터링한다.</p>

<h2>치료 선택지</h2>
<p>갑상선 기능항진증 치료는 수의사와 상의해 강아지의 상태와 생활 환경에 맞는 방법을 선택한다. 일반적으로 약물 치료, 방사성 요오드 치료, 수술, 요오드 제한 처방식 등이 옵션으로 있으며 각각의 장단점이 다르다. 치료 방법 선택과 진행은 반드시 수의사 진단 하에 이루어져야 한다.</p>

<h2>정기 검진이 핵심이다</h2>
<p>10세 이상 고양이는 연 2회 혈액검사를 통해 갑상선 수치를 모니터링하는 것이 WSAVA 권고 사항이다. 증상이 없어도 조기 발견이 치료 반응과 예후를 크게 개선한다. 노령묘 건강 관리는 <a href="/blog/senior-cat-food-guide">노령묘 사료 가이드</a>와 <a href="/blog/cat-kidney-disease-early-signs">신부전 초기 신호</a> 글도 함께 읽어보자.</p>`,
    ymyl: true,
    disclaimer: YMYL_DISCLAIMER,
    status: "published",
  ...
  // ── 67. 강아지 눈 문제 가이드 (cat3, YMYL, macro:A, lens:L4, hook:H4, outro:O3) ──
  {
...
<h3>안구 건조증 (KCS)</h3>
<p>눈물 생성이 부족해지는 질환으로 끈적한 분비물이 특징이다. 코커 스패니얼, 불도그, 시추 등에서 유전적으로 발생률이 높다. 치료하지 않으면 각막 손상으로 이어진다.</p>
...
<ul>
  ...
  <li>사람용 안약, 항생제 안약은 수의사 처방 없이 절대 사용하지 않는다.</li>
</ul>
...
<p>강아지 귀 문제와 눈 문제는 방치하면 빠르게 악화되는 경우가 많다. <a href="/blog/dog-ear-cleaning">강아지 귀 청소 가이드</a>도 함께 참고하자.</p>`,
    ymyl: true,
    disclaimer: YMYL_DISCLAIMER,
    status: "published",
  ...
  // ── 68. 고양이 당뇨 관리 (cat3, YMYL, macro:B, lens:L5, hook:H2, outro:O2) ──
  {
...
<p>고양이 당뇨 관리에서 식단 변경이 가장 중요한 첫 단계다. 건물기준(DM) 탄수화물 10% 이하의 고단백 식단이 혈당 조절에 효과적이다. 습식 사료가 일반적으로 건식 사료보다 탄수화물이 낮다. 당뇨 처방식이나 고단백 습식 사료로의 전환을 수의사와 상의한다. 식단 전환 시 혈당이 빠르게 변할 수 있으므로 인슐린 용량 조절이 필요할 수 있다 — 반드시 수의사 지도 하에 진행한다.</p>
...
<p>진단 초기부터 적극적인 식이 조절과 인슐린 치료를 병행하면 일부 고양이에서 인슐린이 필요 없는 관해 상태에 이를 수 있다. ISFM(국제고양이의학회) 지침에 따르면 진단 후 6개월 이내에 적극 치료를 시작했을 때 관해율이 더 높다. 비만 고양이의 체중 감량도 중요한 요소다.</p>
...
<p>당뇨 관리에 필요한 의료비 부담을 <a href="/insurance/compare">펫보험</a>으로 대비하는 방법도 미리 확인해 두자. 노령묘 식이 관리는 <a href="/blog/senior-cat-food-guide">노령묘 사료 가이드</a>도 함께 참고한다.</p>`,
  ...
  // ── 69. 강아지 외이염 가이드 (cat3, YMYL, macro:B, lens:L4, hook:H4, outro:O3) ──
  {
    ...
    body: `<p>강아지 외이염(otitis externa)은 수의학에서 가장 흔히 진료하는 질환 중 하나다. 미국수의학회(AVMA) 통계에 따르면 매년 전체 강아지 진료의 약 20%가 귀 문제와 관련돼 있다. 방치하면 중이염·내이염으로 진행할 수 있어 조기 대처가 중요하다.</p>
...
<ul>
  ...
  <li><strong>귀진드기(Otodectes)</strong>: 주로 강아지에서 커피색 가루 같은 분비물 — 새끼 강아지에서 흔함</li>
...
<h2>집에서 하는 귀 청소 — 올바른 방법</h2>
<p>이상이 없는 건강한 귀는 정기적인 청소로 외이염을 예방할 수 있다. 단, 이미 염증이 있는 경우 집에서 세정을 하면 오히려 악화될 수 있으므로 동물병원 진단 후에 청소 여부를 결정한다.</p>
<p>귀 청소 방법은 <a href="/blog/dog-ear-cleaning">강아지 귀 청소 완전 가이드</a>에서 단계별로 확인할 수 있다.</p>
...
<h2>재발 예방</h2>
<p>만성 외이염의 경우 근본 원인(알러지)을 찾아 관리하는 것이 핵심이다. 수영 후 귀를 충분히 건조시키고, 귓속 털이 많은 견종은 정기적인 그루밍에서 귀 털 관리를 포함시킨다. 귀 관련 질환은 <a href="/insurance/compare">펫보험</a> 보장 범위에 포함되는 경우가 많으니 확인해 두자.</p>`,
    ymyl: true,
    disclaimer: YMYL_DISCLAIMER,
    status: "published",
  ...
  // ── 70. 고양이 피부 문제 가이드 (cat3, YMYL, macro:B, lens:L4, hook:H2, outro:O3) ──
  {
...
<h3>외부 기생충 (벼룩, 이, 진드기)</h3>
<p>벼룩 알러지 피부염은 고양이 피부 문제의 가장 흔한 원인 중 하나다. 특히 꼬리 기저부나 허리 부분을 격렬하게 긁거나 핥는다면 벼룩을 의심한다. 벼룩 본체는 잘 보이지 않더라도 검은 점 같은 벼룩 배설물(flea dirt)이 털에 있으면 양성이다. 물에 적신 흰 휴지로 배설물을 뭉개면 붉게 변한다(소화된 혈액).</p>
...
<ul>
  <li>탈모 패턴: 대칭성이면 호르몬·스트레스, 비대칭이면 기생충·감염 가능성</li>
...
<ul>
  ...
  <li>원형 탈모 패턴 (링웜 의심)</li>
  <li>집에서 확인하고 싶어도 고양이가 건드리지 못하게 하는 경우</li>
</ul>
<p>피부 문제는 시각적으로 비슷하게 보여도 원인이 전혀 다를 수 있다. 집에서의 외용제 사용은 수의사 지도 없이 하지 않는다. <a href="/condition">질환 정보</a> 페이지에서 고양이 피부 질환 관련 전문 정보를 더 확인할 수 있다.</p>`,
    ymyl: true,
    disclaimer: YMYL_DISCLAIMER,
    status: "published",
